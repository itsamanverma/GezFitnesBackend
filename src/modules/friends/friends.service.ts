...
export const FriendsService = {
  async sendFriendRequest(requesterIdStr: string, recipientEmailOrId: string) {
    let recipient;
  ...
  async acceptFriendRequest(recipientIdStr: string, requestIdStr: string) {
  ...
  async declineFriendRequest(recipientIdStr: string, requestIdStr: string) {
    if (!Types.ObjectId.isValid(requestIdStr)) {
      throw new BadRequestError('Invalid request ID');
    }
  ...
  async listFriends(userIdStr: string) {
  ...
  async listPendingRequests(userIdStr: string) {
    const userId = new Types.ObjectId(userIdStr);
