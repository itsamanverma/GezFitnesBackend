...
export function createSocketServer(httpServer: HttpServer) {
  ...
  io.on('connection', (socket: Socket) => {
    ...
    socket.on(CLIENT_EVENTS.JOIN_SESSION, async (payload: unknown) => {
      try {
    ...
    socket.on(CLIENT_EVENTS.WATCH_SESSION, async (payload: unknown) => {
      try {
    ...
    socket.on(CLIENT_EVENTS.LOCATION_UPDATE, async (payload: unknown) => {
      try {
    ...
    socket.on(CLIENT_EVENTS.PAUSE_SESSION, async (payload: unknown) => {
      try {
    ...
    socket.on(CLIENT_EVENTS.RESUME_SESSION, async (payload: unknown) => {
      try {
        const { sessionId } = SessionIdSchema.parse(payload);
        if (socket.data.role !== 'owner') return;
    ...
    socket.on(CLIENT_EVENTS.END_SESSION, async (payload: unknown) => {
      try {
        const { sessionId } = SessionIdSchema.parse(payload);
        if (socket.data.role !== 'owner') return;
