...
export async function inviteMember(workspaceId, inviterId, email, role) {
    const workspace = await Workspace.findById(workspaceId);
    if (!workspace) {
        throw new NotFoundError('Workspace not found');
    }
...
export async function changeMemberRole(workspaceId, changerId, memberUserId, newRole) {
...
export async function removeMember(workspaceId, changerId, memberUserId) {
    // 1. RBAC Guard on Changer
    const changer = await WorkspaceMember.findOne({ workspaceId, userId: changerId });
    if (!changer || (changer.role !== 'owner' && changer.role !== 'admin')) {
