...
import { NotFoundError, BadRequestError, ConflictError, ForbiddenError, GoneError } from '../../utils/errors.js';
...
export const GroupService = {
    async create(userId, name, type, description, avatar) {
        if (!name || !type) {
    ...
    async join(groupIdStr, userIdStr) {
    ...
    async joinByToken(token, userIdStr) {
        if (!token) {
    ...
    async createInvite(groupIdStr, adminUserIdStr) {
        if (!Types.ObjectId.isValid(groupIdStr)) {
            throw new BadRequestError('Invalid group ID');
        }
        const groupId = new Types.ObjectId(groupIdStr);
    ...
    async list(query) {
        const filter = { type: 'open', isActive: true };
        if (query) {
            filter.name = { $regex: query, $options: 'i' };
        }
        return Group.find(filter).select('-creatorId');
    },
    async getDetails(groupIdStr, userIdStr) {
        if (!Types.ObjectId.isValid(groupIdStr)) {
            throw new BadRequestError('Invalid group ID');
        }
        const groupId = new Types.ObjectId(groupIdStr);
